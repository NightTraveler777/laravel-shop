...
@section('content')
    <div class="tab-content dashboard_content" data-aos="fade-up" data-aos-delay="200">
        <div class="tab-pane fade show active" id="dashboard">
            ...
            <p>На данный момент активен профиль <strong>{{ $active_profile->type }}</strong>.</p>
            <p>Для перехода на другой тип профиля нажмите на соответствующую кнопку.</p>

            @php $types = array_diff($types, [$active_profile->type]); @endphp

            @foreach($types as $type)
                <form action="{{ route('user.profile.switch', ['type' => $type]) }}" method="post" class="switch-profile">
                    @csrf
                    @method('PUT')
